...
import 'package:stela_app/screens/ReportGenerationnine.dart';
...
void main() async {
  ...
  runApp(nineCodingAssessmentExperiment());
}
class nineCodingAssessmentExperiment extends StatefulWidget {
  @override
  _nineCodingAssessmentExperimentState createState() =>
      _nineCodingAssessmentExperimentState();
}
String section1Text = '';
String section2Text = '';
String section3Text = '';
...
class _nineCodingAssessmentExperimentState
    extends State<nineCodingAssessmentExperiment> {
...
final TextEditingController _controller = TextEditingController(text: '''Mean: 3.0, Max: 5, Min: 1, Sum: 15''');
final TextEditingController controller = TextEditingController(text: '''arr = np.array([1, 2, 3, 4, 5])\n''');
...
String universityName = '';
 ...
 String place = '';
  ...
  void initState() {
    ...
    dropdownOptions = [
      [
        DropdownOption(value: 'option1', label: '''import numpy'''),
        DropdownOption(value: 'option2', label: '''from numpy import *'''),
        DropdownOption(value: 'option3', label: '''import numpy as **'''),
        DropdownOption(value: 'option4', label: '''import numpy as np'''),
      ],
      [
        DropdownOption(value: 'option1', label: '''arr = np.array([1, 2, 3, 4, 5])'''),
        DropdownOption(value: 'option2', label: '''arr = numpy.array([1, 2, 3, 4, 5])'''),
        DropdownOption(value: 'option3', label: '''arr = np.array((1, 2, 3, 4, 5))'''),
        DropdownOption(value: 'option4', label: '''arr = np.array{1, 2, 3, 4, 5}'''),
      ],
      [
        DropdownOption(value: 'option1', label: '''mean = np.mean(arr, axis=1)
max_value = np.max(arr, axis=0)
min_value = np.min(arr, axis=1)
sum_value = np.sum(arr, axis=0)'''),
        DropdownOption(value: 'option2', label: '''mean = np.mean(arr, axis=0)
max_value = np.max(arr, axis=0)
min_value = np.min(arr, axis=0)
sum_value = np.sum(arr, axis=0)'''),
        DropdownOption(value: 'option3', label: '''mean = np.mean(arr)
max_value = np.max(arr)
min_value = np.min(arr)
sum_value = np.sum(arr)'''),
        DropdownOption(value: 'option4', label: '''mean = np.mean(arr, axis=0)
max_value = np.max(arr, axis=1)
min_value = np.min(arr, axis=0)
sum_value = np.sum(arr, axis=1)
'''),
      ],
      [
        DropdownOption(value: 'option1', label: '''result_string = f"Mean: {mean}, Max: {max_value}, Min: {min_value}, Sum: {sum_value}"
print(result_string)'''),
        DropdownOption(value: 'option2', label: '''result_string = f"Mean: {mean}, Max: {max_value}, Min: {min_value}, Sum: {sum_value}"
(result_string)'''),
        DropdownOption(value: 'option3', label: '''result_string = f"Mean: {mean}, Max: {max_value}, Min: {min_value}, Sum: {sum_value}"
print[result_string]'''),
        DropdownOption(value: 'option4', label: '''result_string = f"Mean: {mean}, Max: {max_value}, Min: {min_value}, Sum: {sum_value}"
print result_string '''),
      ],
    ];
    selectedOptions = [
...
RichText(
              ...
              Container(
                ...
                SizedBox(height: 10),
                Text(
                  'WAP to implement the basic functionality of Numpy.',
                  style: TextStyle(
                    fontWeight: FontWeight.bold,
                  ),
                ),
  ...
  child: Column(
    ...
    children: [
      Text(
        'Section 1: Import Numpy Library (2 marks)',
        style: TextStyle(
          fontWeight: FontWeight.bold,
        ),
      ),
...
Container(
  ...
  child: Column(
    ...
    children: [
      Text(
        'Section 2: Create Numpy Array, you can input the data as you desire, but do not forget to make the changes in expected output  (2 marks)',
        style: TextStyle(
          fontWeight: FontWeight.bold,
        ),
      ),
...
Container(
  ...
  child: Column(
    ...
    children: [
      Text(
        'Section 3: Perform Operations on Numpy Array (mean, max, min, sum) (2 marks)',
        style: TextStyle(
          fontWeight: FontWeight.bold,
        ),
      ),
...
Container(
  ...
  child: Column(
    ...
    children: [
      Text(
        'Section 4: Display output (2 marks)',
        style: TextStyle(
          fontWeight: FontWeight.bold,
        ),
      ),
...
Container(
  ...
  onPressed: () async {
    ...
    DataSnapshot snapshot = await databaseReference.child('AIPT coding-TEST').child(enrollmentNo).child('Experiment 9').get();
...
int differenceInSeconds = difference.inSeconds%60;
        ...
        '6_Execution Result': executionResult == expectedOutput+"\n" ? 'Correct' : 'Wrong',
                      ...
                      '9_Code' : program,
                      '10_Aim' : 'WAP to implement the basic functionality of Numpy.',
    ...
    /*  } else {
      ...
      setState(() {
                      ...
                      section2Text = section2!='' ? 'Correct' : 'Wrong';
...
PLEASE WAIT FOR A FEW SECONDS TILL YOU SEE THE OUTPUT''',
    style: TextStyle(
      ...
      fontWeight: FontWeight.bold, // Make the text bold
                ...
                SizedBox(height: 20),
                /*Text(
                  'Correct Result: Mean: 3.0, Max: 5, Min: 1, Sum: 15\n',
                  style: TextStyle(fontWeight: FontWeight.bold),
                ),*/
                SizedBox(height: 10),
                Text(
                                              ...
                                              'https://w7.pngwing.com/pngs/827/120/png-transparent-educational-assessment-test-computer-icons-risk-assessment-assess-angle-text-logo-thumbnail.png'),
                                                                        ...
                                                                        TextButton(
                                                                          ...
                                                                          onPressed:
                                                                            ...
                                                                            Navigator.push(
                                                                              context,
                                                                              MaterialPageRoute(builder: (context) => PdfPagenine()),
                                                                            );
                                                                          },
                                                                        ),
],
